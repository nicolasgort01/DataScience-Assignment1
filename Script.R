...
fit_without <- update(fit_rishi, . ~ . - net_trade)
...
# Including net trade is useless. 

# Lets go from Q4. 
# Data Splitting. 

# penalized regression (lasso/ridge/elastic net) is not scale-invariant.
# If predictors aren’t standardized, the penalty treats coefficients equally but their impact on predictions isn’t equal—variables with larger variance can achieve the same fit with a smaller coefficient (hence pay less penalty), so they’re unfairly favored. Standardizing (mean 0, sd 1):
# makes the penalty comparable across features → fair variable selection,
# improves numerical stability and convergence,
# makes hyperparameters (like λ) interpretable/transferable.

n1 <- 110
idx <- sample(1:nrow(data_rishi), n1)
...
# As we can see, more variables are killed if we dont standarize. This happens because if we dont standarize 
# high-variance predictors are favored. We should always standarize to make the penalty fair across all features. 
...
# Create linear regression for only the variables kept. 
library(caret)

# 0) Por si acaso, quita cualquier aparición de la respuesta en kept
kept <- setdiff(kept, "gdp_growth")

stopifnot(length(kept) > 0)  # asegura que haya predictores

# 1) Preprocesa SOLO imputación (para no cambiar unidades)
pp_imp <- preProcess(dfm_train, method = c("medianImpute"))
df_train_lm <- predict(pp_imp, dfm_train)
df_test_lm  <- predict(pp_imp, dfm_test)

# 2) Fórmula con las variables seleccionadas
form_kept <- as.formula(paste("gdp_growth ~", paste(kept, collapse = " + ")))

# 3) Ajusta el modelo lineal
lm_kept <- lm(form_kept, data = df_train_lm)

# 4) Resumen e interpretación
summary(lm_kept)

# 5) Evaluación en test
pred_test <- predict(lm_kept, newdata = df_test_lm)
postResample(pred_test, df_test_lm$gdp_growth)  # RMSE, R^2, MAE


# Now lets do a Lasso with this new set of variables. 

kept <- unique(kept)

data_clean1 <- data_clean %>% 
  select(any_of(kept), gdp_growth)
...
# We transform our variables and run a Lasso again. 

ctrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3,
                     savePredictions = "final")

dfm_train <- data_clean1[idx, ]

dfm_test <- data_clean1[-idx, ]  
...
###### Lasso.
lambda_grid <- 10^seq(-4, 2, length.out = 200)
...
best_lambda_min  <- cv_lasso$lambda.min    # lambda de mínimo CV-MSE
best_lambda_1se  <- cv_lasso$lambda.1se    # regla 1-SE (más parsimonioso)
...
lasso_fit <- glmnet(
  ...
  family = "gaussian",
  alpha = 1,
  lambda = best_lambda_min,         # o best_lambda_1se
  standardize = TRUE
)

# Coeficientes no nulos
beta <- coef(lasso_fit)
nz   <- which(as.numeric(beta) != 0)
print(beta[nz, , drop = FALSE])

lasso_pred <- as.numeric(predict(lasso_fit, newx = Xvars_mat_test))
mse_lasso  <- mean((Yvar_test - lasso_pred)^2)
r2_lasso   <- 1 - sum((Yvar_test - lasso_pred)^2) / sum((Yvar_test - mean(Yvar_test))^2)


### We need to check Lasso again. 
