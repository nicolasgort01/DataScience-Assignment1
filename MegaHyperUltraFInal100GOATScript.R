...
# Data Description. 
average_gdp <- mean(df$`GDP growth (annual %)`)
...
df %>%
  mutate(bin = ntile(`Unemployment, total (% of total labor force) (modeled ILO estimate)`, 10)) %>%
  group_by(bin) %>%
  summarise(
    x = mean(`Unemployment, total (% of total labor force) (modeled ILO estimate)`, na.rm=TRUE),
    y = mean(`GDP growth (annual %)`, na.rm=TRUE)
  ) %>%
  ggplot(aes(x, y)) +
  geom_point() + geom_line() +
  labs(x="Unemployment (%)", y="GDP growth (%)", title="Unemployment vs GDP growth") +
  theme_classic()
